...
import (
	...
	datatransfer "github.com/filtbcoin-project/go-data-transfer"
	...
	"github.com/filtbcoin-project/go-fil-markets/retrievalmarket"

	"github.com/filtbcoin-project/go-address"
	"github.com/filtbcoin-project/go-state-types/abi"
	"github.com/filtbcoin-project/go-state-types/big"

	"github.com/filtbcoin-project/go-fil-markets/storagemarket"

	"github.com/filtbcoin-project/lotus/api"
	lapi "github.com/filtbcoin-project/lotus/api"
	"github.com/filtbcoin-project/lotus/api/v0api"
	"github.com/filtbcoin-project/lotus/build"
	"github.com/filtbcoin-project/lotus/chain/actors/builtin"
	"github.com/filtbcoin-project/lotus/chain/actors/builtin/market"
	"github.com/filtbcoin-project/lotus/chain/types"
	"github.com/filtbcoin-project/lotus/lib/tablewriter"
	"github.com/filtbcoin-project/lotus/node/repo/imports"
)
...
The minimum value is 518400 (6 months).`,
	...
	Flags: []cli.Flag{
		...
		&cli.Int64Flag{
			...
			Usage: "if manually specifying piece cid, used to specify size (dataCid must be to a 

car file)",
		},
		&cli.BoolFlag{
			Name:  "manual-stateless-deal",
			Usage: "instructs the node to send an offline deal without registering it with the 
	...
	Action: func(cctx *cli.Context) error {
		...
		if cctx.Bool("manual-stateless-deal") {
			if ref.TransferType != storagemarket.TTManual || price.Int64() != 0 {
				return xerrors.New("when manual-stateless-deal is enabled, you must also 

provide a 'price' of 0 and specify 'manual-piece-cid' and 'manual-piece-size'")
			}
	...
	for {
		...
		case "duration":
			...
			if days < int(build.MinDealDuration/builtin.EpochsInDay) {
				printErr(xerrors.Errorf("minimum duration is %d days", int

(build.MinDealDuration/builtin.EpochsInDay)))
				continue
			}
...
(time.Millisecond), medianPing.Truncate(time.Millisecond))
			state = "max-ping"
		case "max-ping":
			maxAcceptablePing = medianPing

			afmt.Printf("Maximum network latency (default: %s) (ms): ", 

maxAcceptablePing.Truncate(time.Millisecond))
		...
		case "find-budget":
			...
			for _, ask := range candidateAsks {
				...
				epochPrice := types.BigDiv(types.BigMul(p, types.NewInt(uint64
		...
		pickLoop:
			for i := 0; i < 64; i++ {
				rand.Shuffle(len(candidateAsks), func(i, j int) {
					candidateAsks[i], candidateAsks[j] = candidateAsks[j], 
...
(ds.PieceSize))), gib)
		...
		case "confirm":
			...
			afmt.Printf("\n")
			afmt.Printf("Piece size: %s (Payload size: %s)\n", units.BytesSize(float64

(ds.PieceSize)), units.BytesSize(float64(ds.PayloadSize)))
...
color.YellowString(a.Miner.String()), color.GreenString(types.SizeStr(mpow.MinerPower.QualityAdjPower)), 

color.BlueString(types.FIL(totalPrice).String()), types.FIL(epochPrice))
				}
			}

			// TODO: price is based on PaddedPieceSize, right?
			epochPrice := types.BigDiv(types.BigMul(pricePerGib, types.NewInt(uint64

(ds.PieceSize))), gib)
			totalPrice := types.BigMul(epochPrice, types.NewInt(uint64(epochs)))

			afmt.Printf("Total price: ~%s (%s per epoch)\n", color.CyanString(types.FIL

(totalPrice).String()), types.FIL(epochPrice))
		...
		case "execute":
			...
			for i, maddr := range maddrs {
				...
				afmt.Printf("Deal (%s) CID: %s\n", maddr, color.GreenString(encoder.Encode
...
var clientFindCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		for _, offer := range offers {
			...
			fmt.Printf("RETRIEVAL %s@%s-%s-%s\n", offer.Miner, offer.MinerPeer.ID, types.FIL

(offer.MinPrice), types.SizeStr(types.NewInt(offer.Size)))
		}
	...
	Action: func(cctx *cli.Context) error {
		...
		if watch {
			...
			for {
				...
				err = outputRetrievalDeals(ctx, tm.Screen, localDeals, verbose, showFailed, 
...
completed)
		...
		return outputRetrievalDeals(ctx, cctx.App.Writer, localDeals, verbose, showFailed, 
...
status == retrievalmarket.DealStatusCancelled
}
func outputRetrievalDeals(ctx context.Context, out io.Writer, localDeals []lapi.RetrievalInfo, verbose bool, 

showFailed bool, completed bool) error {
...
var clientDealStatsCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		for _, st := range stateStats {
			...
			fmt.Printf("%s: %d deals, %s\n", storagemarket.DealStates[st.state], st.count, 

types.SizeStr(types.NewInt(st.bytes)))
		}
...
loop2:
	for {
		...
		case <-time.After(150 * time.Millisecond):
			if isTTY {
				fmt.Printf("\r* Queried %d asks, got %d responses", atomic.LoadInt64

(&queried), atomic.LoadInt64(&got))
			}
		case <-done:
			break loop2
		}
...
var clientQueryAskCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		perEpoch := types.BigDiv(types.BigMul(ask.Price, types.NewInt(uint64(size))), types.NewInt
...
(1<<30))
		...
		afmt.Printf("Total Price: %s\n", types.FIL(types.BigMul(perEpoch, types.NewInt(uint64
	...
	Action: func(cctx *cli.Context) error {
		...
		if watch {
			...
			for {
				...
				err = outputStorageDeals(ctx, tm.Screen, api, localDeals, verbose, 
...
func outputStorageDeals(ctx context.Context, out io.Writer, full v0api.FullNode, localDeals []lapi.DealInfo, 

verbose bool, showFailed bool) error {
	...
	if verbose {
		...
		fmt.Fprintf(w, "Created\tDealCid\tDealId\tProvider\tState\tOn Chain?\tSlashed?\tPieceCID

\tSize\tPrice\tDuration\tTransferChannelID\tTransferStatus\tVerified\tMessage\n")
		for _, d := range deals {
			onChain := "N"
			...
			price := types.FIL(types.BigMul(d.LocalDeal.PricePerEpoch, types.NewInt
...
d.LocalDeal.DataTransfer.Transferred, d.LocalDeal.Size)
				...
				//	pct := (100 * d.LocalDeal.DataTransfer.Transferred) / 
	...
	Action: func(cctx *cli.Context) error {
		...
		if watch {
			...
			for {
				...
				OutputDataTransferChannels(tm.Screen, channels, verbose, completed, 
...
func OutputDataTransferChannels(out io.Writer, channels []lapi.DataTransferChannel, verbose, completed, 
...
showFailed bool) {
	...
	for _, channel := range channels {
		...
		if !showFailed && (channel.Status == datatransfer.Failed || channel.Status == 
...
func toChannelOutput(otherPartyColumn string, channel lapi.DataTransferChannel, verbose bool) map
...
func renderDeal(di *lapi.DealInfo) {
	...
	if di.DealStages == nil {
		color.Yellow("Deal was made with an older version of Lotus and Lotus did not collect 

detailed information about its stages")
		return
	}

	for _, stg := range di.DealStages.Stages {
		msg := fmt.Sprintf("%s %s: %s (expected duration: %s)", color.BlueString("Stage:"), 

color.BlueString(strings.TrimPrefix(stg.Name, "StorageDeal")), stg.Description, color.GreenString

(stg.ExpectedDuration))
		...
		for _, l := range stg.Logs {
			fmt.Printf("  %s %s\n", color.YellowString(l.UpdatedTime.Time().UTC().Round

(time.Second).Format(time.Stamp)), l.Log)
		}

		if stg.Name == "StorageDealStartDataTransfer" {
			for _, dtStg := range di.DataTransfer.Stages.Stages {
				fmt.Printf("        %s %s %s\n", color.YellowString(dtStg.CreatedTime.Time

().UTC().Round(time.Second).Format(time.Stamp)), color.BlueString("Data transfer stage:"), color.BlueString

(dtStg.Name))
				for _, l := range dtStg.Logs {
					fmt.Printf("              %s %s\n", color.YellowString

(l.UpdatedTime.Time().UTC().Round(time.Second).Format(time.Stamp)), l.Log)
				}
			}
		}
	}
